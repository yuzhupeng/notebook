...
ACTION_CONVERT = ["lazyida:convert%d" % i for i in range(11)]
        ...
        if self.action in ACTION_CONVERT:
            ...
            if data:
                ...
                if self.action == ACTION_CONVERT[0]:
                    ...
                    print("Start Magic")
                    console = sys.stdin
                    dump_fp     =   open(r"dump.txt",'w')
                    src_fp    =   open(r"input.txt", 'r')
                    sys.stdin = src_fp
                    mod = input()                
                    src = int(input(),16)                 
                    des = int(input(),16)                
...
class LazyIDA_t(idaapi.plugin_t):
    ...
    def init(self):
        ...
        menu_actions = (
            ...
            idaapi.action_desc_t(ACTION_CONVERT[10], "C/C++ from src to dec (B/W/D/Q)",
                                menu_action_handler_t(ACTION_CONVERT[10]), None, None, 38),
